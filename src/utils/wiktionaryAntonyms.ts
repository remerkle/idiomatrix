...
const CACHE_KEY = 'idiomatrix-wiktionary-antonyms';
...
function findAntonyms(wikitext: string, languageId: string, word: string): string[] | null {
  ...
  // Modern entries list antonyms inline per-sense: {{ant|xx|word1|word2|...}}.
  // Take the first one found — later senses belong to other parts of speech
  // and mixing them in produces unrelated results.
  for (const m of section.matchAll(/\{\{ant\|([a-z-]+)\|([^}]+)\}\}/g)) {
  ...
  // Fall back to an older-style dedicated ===Antonyms=== heading with [[links]]
  const block = section.match(/={2,5}\s*Antonyms\s*={2,5}([\s\S]*?)(?:\n=|$)/i);
  if (block) {
...
export async function lookupWiktionaryAntonyms(
  word: string,
  languageId: string,
): Promise<string[] | null> {
  ...
  const result = wikitext ? findAntonyms(wikitext, languageId, word) : null;
